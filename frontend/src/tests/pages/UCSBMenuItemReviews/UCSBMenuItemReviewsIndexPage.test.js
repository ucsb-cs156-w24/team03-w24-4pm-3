...
describe("UCSBMenuItemReviewsIndexPage tests", () => {
    ...
    test("Renders with Create Button for admin user", async () => {
        ...
        axiosMock.onGet("/api/ucsbmenuitemreviews/all").reply(200, []);

        // act
        render(
            <QueryClientProvider client={queryClient}>
                <MemoryRouter>
                    <UCSBMenuItemReviewsIndexPage />
                </MemoryRouter>
            </QueryClientProvider>
        );
    ...
    test("renders three menuitemreviews correctly for regular user", async () => {
        ...
        setupUserOnly();
        const queryClient = new QueryClient();
        axiosMock.onGet("/api/ucsbmenuitemreviews/all").reply(200, ucsbMenuItemReviewsFixtures.threeMenuItemReviews);
    ...
    test("renders empty table when backend unavailable, user only", async () => {
        ...
        setupUserOnly();
        const queryClient = new QueryClient();
        axiosMock.onGet("/api/ucsbmenuitemreviews/all").timeout();
        ...
        expect(errorMessage).toMatch("Error communicating with backend via GET on /api/ucsbmenuitemreviews/all");
        restoreConsole();
    ...
    test("what happens when you click delete, admin", async () => {
        ...
        const queryClient = new QueryClient();
        axiosMock.onGet("/api/ucsbmenuitemreviews/all").reply(200, ucsbMenuItemReviewsFixtures.threeMenuItemReviews);
        axiosMock.onDelete("/api/ucsbmenuitemreviews").reply(200, "MenuItemReview with id 1 was deleted");
